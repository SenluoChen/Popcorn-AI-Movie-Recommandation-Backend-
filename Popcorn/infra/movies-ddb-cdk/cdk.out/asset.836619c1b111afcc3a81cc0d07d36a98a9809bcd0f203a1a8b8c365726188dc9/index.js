...
exports.handler = async (event) => {
  ...
  try {
    // Start DynamoDB scan in parallel with OpenAI work to reduce wall-clock time.
    const tScanStart = nowMs();
    const moviesPromise = scanAllMovies({ tableName, region, maxItems: Number.isFinite(maxScan) && maxScan > 0 ? Math.floor(maxScan) : null });
    ...
    const tScanMs = nowMs() - tScanStart;
