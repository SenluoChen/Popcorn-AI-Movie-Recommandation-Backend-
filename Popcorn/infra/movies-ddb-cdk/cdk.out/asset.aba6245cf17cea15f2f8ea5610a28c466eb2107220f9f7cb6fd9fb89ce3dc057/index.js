...
function movieMatchesHints(movie, hints) {
  ...
  if (hints?.wantLang) {
    ...
    // If the user explicitly asked for a specific language/country (e.g. 日本電影 / Korean movies),
    // treat it as a hard constraint: do NOT allow unknown language unless we have matching country.
    const strictLang = Boolean(
      (want === 'ja' && hints?.wantsJapanese)
      || (want === 'ko' && hints?.wantsKorean)
      || (want === 'en' && hints?.wantsEnglish)
    );
    ...
    if (langCode) {
      ...
      // Unknown language: only accept when the production country strongly matches the intent.
      if (want === 'ja') {
        if (!country.includes('japan')) return false;
      } else if (want === 'ko') {
        if (!country.includes('korea')) return false;
      } else if (want === 'en') {
        // English intent isn't tied to a single country; keep lenient.
      }
    }
  }
